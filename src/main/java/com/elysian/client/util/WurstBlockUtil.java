...
public class WurstBlockUtil implements Globals {
    ...
    public static boolean isValidBlock(BlockPos pos) {
        Block block = WurstBlockUtil.mc.world.getBlockState(pos).getBlock();
    ...
    public static int isPositionPlaceable(WurstBlockUtil pos, boolean rayTrace) {
        return WurstBlockUtil.isPositionPlaceable(pos, rayTrace, true);
    }
    ...
    public static int isPositionPlaceable(BlockPos pos, boolean rayTrace, boolean entityCheck) {
        ...
        if (!WurstBlockUtil.rayTracePlaceCheck(pos, rayTrace, 0.0f)) {
            return -1;
        }
        if (entityCheck) {
            for (Entity entity : WurstBlockUtil.mc.world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(pos))) {
                if (entity instanceof EntityItem || entity instanceof EntityXPOrb) continue;
                return 1;
            }
        }
        for (EnumFacing side : WurstBlockUtil.getPossibleSides(pos)) {
            if (!WurstBlockUtil.canBeClicked(pos.offset(side))) continue;
            return 3;
        }
        return 2;
    }
    ...
    public static boolean canBeClicked(BlockPos pos) {
        return WurstBlockUtil.getBlock(pos).canCollideCheck(WurstBlockUtil.getState(pos), false);
    }

    private static Block getBlock(BlockPos pos) {
        return WurstBlockUtil.getState(pos).getBlock();
    }
