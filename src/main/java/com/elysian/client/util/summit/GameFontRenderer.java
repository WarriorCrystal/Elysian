...
public class GameFontRenderer extends FontRenderer
{
    ...
    public GameFontRenderer(final Font font) {
        ...
        boldFont = new AWTFontRenderer(font.deriveFont(1));
        italicFont = new AWTFontRenderer(font.deriveFont(2));
        boldItalicFont = new AWTFontRenderer(font.deriveFont(3));
    ...
    public void drawCenteredString(final String s, final float x, final float y, final int color, final boolean shadow) {
        drawString(s, x - getStringWidth(s) / 2.0f, y, color, shadow);
    }
    
    public void drawCenteredString(final String s, final float x, final float y, final int color) {
        drawString(s, x - getStringWidth(s) / 2.0f, y, color);
    }
    ...
    public int drawString(final String text, final float x, final float y, final int color, final boolean dropShadow) {
        ...
        if (dropShadow) {
            ...
            drawText(text, x + 0.5f, currY + 0.5f, new Color(0, 0, 0, alpha * 150).getRGB(), false);
        }
            ...
            for (int index = 0; index < parts.length; ++index) {
                ...
                if (!part.isEmpty()) {
                    if (index == 0) {
                        ...
                        if (strikeThrough) {
                            RenderUtil.drawLine(width / 2.0 + 1.0, currentFont.getHeight() / 3.0, (width + currentFont.getStringWidth(words)) / 2.0 + 1.0, currentFont.getHeight() / 3.0, FONT_HEIGHT / 16.0f);
                        }
                        if (underline) {
                            RenderUtil.drawLine(width / 2.0 + 1.0, currentFont.getHeight() / 2.0, (width + currentFont.getStringWidth(words)) / 2.0 + 1.0, currentFont.getHeight() / 2.0, FONT_HEIGHT / 16.0f);
                        }
