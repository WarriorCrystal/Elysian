...
public class ColorUtil
{
    ...
    public ColorUtil(final float[] d, final float k) {
        m_HSB = d;
        m_Alpha = k;
        m_BaseColor = GetRainbowColorFromArray(d, k);
    }
    ...
    public ColorUtil(final float n, final float n2, final float n3, final float k) {
        ...
        m_HSB = d;
        m_Alpha = k;
        m_BaseColor = GetRainbowColorFromArray(m_HSB, k);
    }
    
    @Override
    public String toString() {
        return new StringBuilder().insert(0, "HSLColor[h=").append(m_HSB[0]).append(",s=").append(m_HSB[1]).append(",l=").append(m_HSB[2]).append(",alpha=").append(m_Alpha).append("]").toString();
    }
    
    public Color GetColorWithLightnessMax(float max) {
        max = (100.0f - max) / 100.0f;
        max = Math.max(0.0f, m_HSB[2] * max);
        return GetRainbowColor(m_HSB[0], m_HSB[1], max, m_Alpha);
    }
    
    public Color GetColorWithLightnessMin(float min) {
        min = (100.0f + min) / 100.0f;
        min = Math.min(100.0f, m_HSB[2] * min);
        return GetRainbowColor(m_HSB[0], m_HSB[1], min, m_Alpha);
    }
    ...
    public Color GetColorWithBrightness(final float p_Brightness) {
        return GetRainbowColor(m_HSB[0], m_HSB[1], p_Brightness, m_Alpha);
    }
    ...
    public Color GetColorWithHue(final float p_Hue) {
        return GetRainbowColor(p_Hue, m_HSB[1], m_HSB[2], m_Alpha);
    }
    
    public Color GetColorWithSaturation(final float p_Saturation) {
        return GetRainbowColor(m_HSB[0], p_Saturation, m_HSB[2], m_Alpha);
    }
    ...
    public Color GetColorWithModifiedHue() {
        return ColorRainbowWithDefaultAlpha((m_HSB[0] + 180.0f) % 360.0f, m_HSB[1], m_HSB[2]);
    }
